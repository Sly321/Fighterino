...
void UIOverlay::drawInterface(QPainter *p, Character *c,  Character *e) {
    ...
    p->drawImage(15, 15,c->getIcon().scaled(40,40));       // Picture Character
    ...
    p->setPen(QPen(Qt::white));
    p->setBrush(QBrush(Qt::darkMagenta));
    p->drawImage(745, 15, e->getIcon().scaled(40,40).mirrored(true, false));       // Picture c

    p->setPen(QPen(Qt::darkRed));
    p->setBrush(QBrush(Qt::darkRed));
    p->drawRect(QRect(590, 15, 150, 15)); // Healthbar All
    p->setBrush(QBrush(Qt::red));
    p->drawRect(QRect(590, 15, e->getLife(), 15)); // Healthbar Current
    p->setPen(QPen(Qt::white));
    p->drawText(QRect(590, 15, 150, 15), QString::number(e->getLife()) + " / 150", QTextOption(Qt::AlignCenter)); // Healthtext

    p->setPen(QPen(Qt::darkBlue));
    p->setBrush(QBrush(Qt::darkBlue));
    p->drawRect(QRect(590, 40, 150, 15)); // Manabar All
    p->setBrush(QBrush(Qt::blue));
    p->drawRect(QRect(590, 40,e->getMana(), 15)); // Manabar Current
    p->setPen(QPen(Qt::white));
    p->drawText(QRect(590, 40, 150, 15), QString::number(e->getMana()) + " / 150", QTextOption(Qt::AlignCenter)); // Manatext
